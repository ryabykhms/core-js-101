...
function doRectanglesOverlap(rect1, rect2) {
  ...
  const isHorizontOverlap = rect1.left < right2 && right1 > rect2.left;
  const isVerticalOverlap = rect1.top < bottom2 && bottom1 > rect2.top;

  return isHorizontOverlap && isVerticalOverlap;
}
...
function isInsideCircle(circle, point) {
  ...
  const distance = Math.sqrt(section.x ** 2 + section.y ** 2);
...
function findFirstSingleChar(str) {
  const reps = str.split('').reduce((acc, cur) => {
    if (!acc[cur]) {
      acc[cur] = {
        reps: 1,
      };
  ...
  return Object.keys(reps).find((item) => reps[item].reps === 1) || null;
}
...
function getCommonDirectoryPath(pathes) {
  const pathesArr = pathes.reduce((acc, item) => [...acc, item.split('/')], []);
  const firstPath = pathesArr[0];
  const common = [];

  firstPath.forEach((item, i) => {
    const isCommon = pathesArr.every((path) => path[i] === item);
    if (isCommon) {
      common.push(item);
    }
  });
